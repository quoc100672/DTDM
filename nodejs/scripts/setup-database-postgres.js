...
async function createDatabase() {
  ...
  try {
    ...
    console.log('✓ Đã kết nối PostgreSQL server');
    ...
    // Tạo index
    await pool.query('CREATE INDEX idx_name ON products(name)');
    await pool.query('CREATE INDEX idx_created_at ON products(created_at)');
    ...
    console.log(`✓ Tổng số sản phẩm trong database: ${result.rows[0].total}`);

    console.log('\n✅ HOÀN THÀNH! PostgreSQL database đã sẵn sàng sử dụng.');
